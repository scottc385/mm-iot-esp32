...
#ifndef CONFIG_ROUTER_STA_W5500_BIP_MAX_APDU_CODE
#define CONFIG_ROUTER_STA_W5500_BIP_MAX_APDU_CODE 5
#endif
...
static bool parse_ipv4(const char *text, uint32_t *addr)
{
    struct in_addr parsed = {0};
    if (!text || !addr || inet_aton(text, &parsed) == 0) {
        return false;
    }
    *addr = parsed.s_addr;
    return true;
...
static uint32_t derive_broadcast_addr(uint32_t ip_addr, uint32_t netmask)
{
    uint32_t ip_host = ntohl(ip_addr);
    uint32_t mask_host = ntohl(netmask);
    return htonl((ip_host & mask_host) | ~mask_host);
}

static uint32_t bip_port_broadcast_addr(void)
{
    uint32_t ip_addr = 0;
    uint32_t netmask = 0;
    if (w5500_probe_get_ip_info(&ip_addr, &netmask, NULL)) {
        return derive_broadcast_addr(ip_addr, netmask);
    }

    const router_runtime_config *cfg = router_runtime_config_get();
    if (cfg &&
        parse_ipv4(cfg->w5500_ip, &ip_addr) &&
        parse_ipv4(cfg->w5500_netmask, &netmask)) {
        return derive_broadcast_addr(ip_addr, netmask);
    }

    return parse_ipv4_or_broadcast(CONFIG_ROUTER_STA_W5500_BIP_BROADCAST_ADDR);
}
...
static void bip_port_log_send_error(const bip_port *port,
                                    ...
                                    int send_errno)
{
    ...
    if (!port || port->sock < 0 ||
        getsockopt(port->sock, SOL_SOCKET, SO_ERROR, &so_error, &so_error_len) != 0) {
        so_error = -1;
    }

    printf("TX_BIP_ERR errno=%d so_error=%d net=%u target=%s:%u function=0x%02x npdu_len=%u bvlc_len=%u tx_errors=%lu w5500_ip=%u\n",
           send_errno,
           so_error,
           port ? (unsigned)port->net : 0U,
           target ? inet_ntoa(target->sin_addr) : "0.0.0.0",
           target ? (unsigned)ntohs(target->sin_port) : 0U,
           (unsigned)function,
           (unsigned)npdu_len,
           (unsigned)frame_len,
           port ? (unsigned long)port->tx_errors : 0UL,
           w5500_probe_has_ip() ? 1U : 0U);
}

static void bip_port_clamp_confirmed_req_max_apdu(uint8_t *npdu, size_t npdu_len)
{
    if (!npdu || npdu_len == 0 || CONFIG_ROUTER_STA_W5500_BIP_MAX_APDU_CODE >= 5) {
        return;
    }
    ...
    if (apdu_offset <= 0 || npdu_data.network_layer_message ||
        (size_t)apdu_offset + 2u > npdu_len) {
        return;
    }

    uint8_t *apdu = &npdu[apdu_offset];
    if ((apdu[0] & 0xF0u) != PDU_TYPE_CONFIRMED_SERVICE_REQUEST) {
        return;
    }

    uint8_t old_code = apdu[1] & 0x0fu;
    uint8_t max_code = (uint8_t)CONFIG_ROUTER_STA_W5500_BIP_MAX_APDU_CODE;
    if (old_code > max_code) {
        apdu[1] = (uint8_t)((apdu[1] & 0xF0u) | max_code);
        if (router_log_get(ROUTER_LOG_SEGMENT_DIAG)) {
            ESP_LOGI(TAG,
                     "TX_BIP_MAX_APDU_CLAMP old=%u new=%u npdu_len=%u",
                     (unsigned)old_code,
                     (unsigned)max_code,
                     (unsigned)npdu_len);
        }
                                  ...
                                  uint8_t function)
{
    ...
    bip_port_clamp_confirmed_req_max_apdu(&port->tx_frame[BVLC_HEADER_LEN], npdu_len);
    const uint8_t *tx_npdu = &port->tx_frame[BVLC_HEADER_LEN];
    ...
    int send_errno = sent == (int)frame_len ? 0 : errno;
    ...
    ESP_LOGI(TAG,
             ...
             sent,
             send_errno);
    router_seg_diag_log_npdu("TX_BIP_NPDU", tx_npdu, npdu_len);
    ...
    if (sent != (int)frame_len) {
        port->tx_errors++;
        bip_port_log_send_error(port, target, function, npdu_len, frame_len, send_errno);
        return;
    }

    port->tx_frames++;
    router_bacnet_trace_npdu("tx", "bip", tx_npdu, npdu_len);
...
void bip_port_send_npdu_broadcast(bip_port *port, const uint8_t *npdu, size_t npdu_len)
{
    ...
    target.sin_addr.s_addr = bip_port_broadcast_addr();
                   ...
                   uint32_t now_ms)
{
    ...
    while (true) {
        ...
        port->rx_frames++;
        router_bacnet_trace_npdu("rx", "bip", npdu, npdu_len);
#if ROUTER_SEG_DIAG_ENABLE
        if (router_log_get(ROUTER_LOG_SEGMENT_DIAG)) {
        ESP_LOGI(TAG,
