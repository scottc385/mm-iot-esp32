...
__attribute__((weak)) void mstp_debug_frame(
    bool tx, uint8_t frame_type, uint8_t src, uint8_t dst, uint16_t len)
{
    (void)tx;
    (void)frame_type;
    (void)src;
    ...
    uint16_t data_len)
{
    ...
    mstp_debug_frame(true, frame_type, source, destination, data_len);
            ...
            if (mstp_port->SilenceTimer((void *)mstp_port) >
                ...
                if (mstp_port->Index == 0) {
                    ...
                    if (mstp_port->DataLength == 0) {
                        /* NoData */
                        mstp_debug_frame(false, mstp_port->FrameType,
